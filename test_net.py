...
class HybridGConvBlock(nn.Module):
    def __init__(self, 
    ...
    def forward(self, features_list: list, labels: torch.Tensor):
        ...
        # 类内和类间邻接矩阵
        in_class_edge_index = utils.in_class_adj_matrix(labels)
        cross_class_edge_index = utils.cross_class_adj_matrix(labels)
...
class HybridStackedGConvNet(nn.Module):
    def __init__(self, 
                 num_feature_inputs, feature_embed_sizes, 
                 final_embed_size, cross_class_conv1d_kernel_size, cross_class_conv1d_stride,
                 dropout_rate, num_classes_output, query_size, num_features=2, gat_num_heads=1,
                 num_hybrid_conv_blocks=2):
        super().__init__()
        self.num_features = num_features
        self.dropout = nn.Dropout(p=dropout_rate)
        self.num_classes_output = num_classes_output
        self.query_size = query_size
        self.num_hybrid_conv_blocks = num_hybrid_conv_blocks

        self.gconv_blks = nn.Sequential()
        for i in range(num_hybrid_conv_blocks):
            self.gconv_blks.add_module("gconv_blk"+str(i),
                                       HybridGConvBlock(num_feature_inputs, feature_embed_sizes,
                                                        final_embed_size, cross_class_conv1d_kernel_size,
                                                        cross_class_conv1d_stride,
                                                        dropout_rate, num_classes_output,
                                                        query_size, num_features, gat_num_heads))
    ...
    def forward(self, features_list: list, labels: torch.Tensor):
        ...
        for gconv_blk in self.gconv_blks:
            output = gconv_blk(features_list, temp_labels)
            temp_labels = output.argmax(axis=-1)
